...
def get_tutorials_by_coursecode(code):
    ...
    #                                    "(SELECT t1.group_num, count(*) AS stuAmount FROM Tutorials t1 NATURAL JOIN Attend a1 WHERE t1.code= %(code)s AND t1.code = a1.code GROUP BY t1.group_num) as stu "
    #                                    "NATURAL JOIN"
    #                                    "(SELECT t2.group_num, count(*) AS taAmount FROM Tutorials t2 NATURAL JOIN Facilitate f1 WHERE t2.code= %(code)s AND t2.code = f1.code GROUP BY t2.group_num) as ta "
    #                                    , {'code': code})
